...
func TestSavedCompositionEvidenceDependencyClosure(t *testing.T) {
	...
	if audit.Verdict != "pass" || audit.CoreCommit != evidenceDependencyCoreCommit || audit.Inputs != 5 || audit.Outputs != 19 || audit.Runs != 3 || audit.ChangedInputs != 0 || audit.AffectedOutputs != 0 || audit.CompletionState != "incomplete" || audit.DefinitiveEligible || !sameSet(audit.Gaps, []string{"process-executable-attestation-unavailable", "subject-depth-parity-incomplete", "subject-v2-certificate-atomic-binding-unavailable", "surface-pattern-proof-gaps"}) {
...
func TestCompositionEvidenceDependencyNegativeMatrix(t *testing.T) {
	...
	if report.Verdict != "pass" || report.CoreCommit != evidenceDependencyCoreCommit || len(report.Results) != 10 {
