...
`go run ./cmd/atlas-lab composition-compatibility-matrix`は24 consumer/case結果を`evidence/preview/composition-compatibility.matrix.json`へ保存します。`go run ./cmd/atlas-lab preview-publication-gate`は既存v1 Publication baseline、Depth Gap継承、複数Subject Matrix、Router v2 Eval、中立表現を横断します。これはdraft Preview Gateであり、v1 Completion Certificateの内容や状態を変更しません。
