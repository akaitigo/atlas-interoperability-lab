...
公開CIの`public-ci-gate`は署名、固定commit／source path／digest、report、lock、preview、workflow、`repo.yaml`に加え、validator本体、CLI routing、allowed-signers trust root、DCO range verifier、Gitless隔離copyのBaseline reference root検証をfail-closed検証します。署名・Digest改ざん、trust root追加、routing迂回、DCO verifier緩和、reference root検証の改変、各既存Gate command削除、private clone再導入の20 negative fixtureを同じ入口で拒否します。
